...
class TiTmCn2R2C_summer_V5bis(DarkGreyModel):
    """
    Grey-box model with Kalman Filter for occupancy estimation.
    ...
    Ti  : Indoor air temperature (°C)
    Tm  : Lumped thermal mass temperature (°C)
    c   : Indoor CO2 concentration (ppm) - forward simulated
    N   : Effective occupancy (persons) - updated via Kalman filter
    ...
    -------------------
    Ti0, Tm0, c0, N0 : Initial states
    P_N_init : Initial variance of N estimate [persons²]
    Ci, Cm   : Air and mass capacitances [J/K]
    Rim, Rout: Thermal resistances [K/W]
    rho_air, cp_air : Air properties
    V, S, A  : Room volume [m³], surface [m²], window area [m²]
    G        : CO2 emission per person [m³/h/person]
    c_out    : Outdoor CO2 [ppm]
    q_pers, q_equip_var, q_equip_const : Internal gain parameters
    g        : Solar transmittance [-]
    Q_N      : Process noise variance for N [persons²] — how fast N can change
    R_C      : Observation noise variance on N-from-CO2 [persons²]
               (absorbs both CO2 sensor noise and steady-state inversion error)
    """
    ...
    def model(self, params, X):
        ...
        c   = np.zeros(num_rec)
        N   = np.zeros(num_rec)
        P_N = np.zeros(num_rec)  # Variance of N estimate [persons²]
        ...
        q_equip_const = params['q_equip_const'].value
        V             = params['V'].value
        S_room        = params['S'].value   # FIX: renamed from S to S_room
        A             = params['A'].value
        G             = params['G'].value
        c_out         = params['c_out'].value
        rho_air       = params['rho_air'].value
        ...
        # Kalman filter noise parameters
        Q_N = params['Q_N'].value if 'Q_N' in params else 0.1   # persons²
        R_C = params['R_C'].value if 'R_C' in params else 0.25  # persons²
        ...
        for k in range(1, num_rec):
            ...
            # 2) Internal gains — uses S_room (FIX: was overwritten by S below)
            Q_int[k] = (q_pers + q_equip_var) * N[k-1] + q_equip_const * S_room
            ...
            # ── KALMAN FILTER FOR N ─────────────────────────────────────────
            #
            # Process model:  N_k = N_{k-1} + v_k,  v_k ~ N(0, Q_N * dt)
            # Measurement:    N_meas,k = N_k + w_k,  w_k ~ N(0, R_C)

            # PREDICT
            N_pred = N[k-1]                        # N̂_{k|k-1} = N_{k-1}
            P_pred = P_N[k-1] + Q_N * dt           # P_{k|k-1} = P_{k-1} + Q_N*dt

            # CO2-DERIVED MEASUREMENT (steady-state inversion)
            # N_meas = (qv_{k-1} / G*10^6) * (c_meas_{k-1} - c_out)
            if qv[k-1] > 0:
                N_meas = max(0.0, (qv[k-1] / (G * 1e6)) * (c_meas[k-1] - c_out))
            else:
                N_meas = 0.0

            # INNOVATION
            y_innov = N_meas - N_pred              # ỹ_k = N_meas,k - N̂_{k|k-1}

            # INNOVATION COVARIANCE  (FIX: renamed from S to S_innov)
            S_innov = P_pred + R_C                 # S_k = P_{k|k-1} + R_C

            # KALMAN GAIN
            K_gain = P_pred / S_innov if S_innov > 1e-10 else 0.0  # K_k = P_{k|k-1} / S_k

            # STATE UPDATE
            N[k]    = max(0.0, N_pred + K_gain * y_innov)  # N̂_k = N̂_{k|k-1} + K_k * ỹ_k

            # COVARIANCE UPDATE
            P_N[k]  = (1.0 - K_gain) * P_pred     # P_k = (1 - K_k) * P_{k|k-1}

            # ── CO2 FORWARD MODEL (FIX: restored, was replaced by c[k]=c_meas[k])
            # Uses updated N[k] so CO2 simulation is consistent with occupancy estimate
            dc   = (1e6 * (G / V) * N[k] - qv[k] / V * (c[k-1] - c_out)) * dt
            c[k] = c[k-1] + dc
